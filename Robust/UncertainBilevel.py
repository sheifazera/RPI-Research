...
Master Problem: (P9)
    ...
    s.t. AR*xu + AZ*yu + BR*xl0 + BZ* yl0 <= r (12)
        AR*xu + AZ*yu + BR*xl0 + BZ* yl0 +||P^T[x;y]||<= r (12)
...
def Master_cR1(Master,j):
    value=sum(Parent.Px[i,j]*Parent.Master.xu[i] for i in Parent.mRset)
    return value==Parent.Master.PX[j]
Parent.Master.cR1=Constraint(Parent.mRset,rule=Master_cR1)

def Master_cR1b(Master,j):
    value=sum(Parent.Py[i,j]*Parent.Master.yu[i] for i in Parent.mZset)
    return value==Parent.Master.PY[j]
...
def Master_cR3(Master,j):
    value=(sum(Parent.AR[j,i]*Parent.Master.xu[i] for i in Parent.mRset)+
          sum(Parent.AZ[j,i]*Parent.Master.yu[i] for i in Parent.mZset)+
          Parent.Master.v+
          sum(Parent.BR[(j,i)]*Parent.Master.xl0[i] for i in Parent.nRset)+
          sum(Parent.BZ[(j,i)]*Parent.Master.yl0[i] for i in Parent.nZset))
    return value - Parent.r[j] <= Parent.zero
Parent.Master.cR3=Constraint(Parent.mUset,rule=Master_cR3)
...
def sub2_cR3(sub2,j):
    ...
    return value - Parent.r[j] <= Parent.zero
Parent.sub2.cR3=Constraint(Parent.mUset,rule=sub2_cR3)
